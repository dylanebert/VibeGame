import { describe, expect, it } from 'bun:test';
import { horizontalStrategy } from 'vibegame/arrange';

describe('Horizontal Strategy', () => {
  it('should center single member at origin', () => {
    const pos = horizontalStrategy(1, 2, 0);
    expect(pos.x).toBe(0);
    expect(pos.y).toBe(0);
    expect(pos.z).toBe(0);
  });

  it('should distribute two members symmetrically', () => {
    const pos0 = horizontalStrategy(2, 4, 0);
    const pos1 = horizontalStrategy(2, 4, 1);

    expect(pos0.x).toBe(-2);
    expect(pos1.x).toBe(2);
  });

  it('should distribute three members with correct spacing', () => {
    const pos0 = horizontalStrategy(3, 3, 0);
    const pos1 = horizontalStrategy(3, 3, 1);
    const pos2 = horizontalStrategy(3, 3, 2);

    expect(pos0.x).toBe(-3);
    expect(pos1.x).toBe(0);
    expect(pos2.x).toBe(3);
  });

  it('should respect gap parameter', () => {
    const pos0 = horizontalStrategy(2, 10, 0);
    const pos1 = horizontalStrategy(2, 10, 1);

    expect(pos1.x - pos0.x).toBe(10);
  });

  it('should handle zero gap', () => {
    const pos0 = horizontalStrategy(3, 0, 0);
    const pos1 = horizontalStrategy(3, 0, 1);
    const pos2 = horizontalStrategy(3, 0, 2);

    expect(pos0.x).toBe(0);
    expect(pos1.x).toBe(0);
    expect(pos2.x).toBe(0);
  });

  it('should keep y and z at zero', () => {
    for (let i = 0; i < 5; i++) {
      const pos = horizontalStrategy(5, 2, i);
      expect(pos.y).toBe(0);
      expect(pos.z).toBe(0);
    }
  });

  it('should return correct positions for 5 members with gap 2', () => {
    // Total width: (5-1) * 2 = 8
    // Start: -4
    // Positions: -4, -2, 0, 2, 4
    const positions = [0, 1, 2, 3, 4].map((i) => horizontalStrategy(5, 2, i));

    expect(positions[0].x).toBe(-4);
    expect(positions[1].x).toBe(-2);
    expect(positions[2].x).toBe(0);
    expect(positions[3].x).toBe(2);
    expect(positions[4].x).toBe(4);
  });
});
